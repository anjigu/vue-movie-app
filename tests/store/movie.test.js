...
describe('store/movie.js', () => {
    ...
    test('영화 데이터를 초기화합니다', () => {
        ...
        expect(store.state.message).toBe('Search fot the movie title!')
...
test('영화 목록을 잘 가져오지 못한 경우 에러 메시지를 확인합니다', async() => {
...
test('영화 아이템이 중복된 경우 고유하게 처리합니다', () => {
    const res = {
        data: {
            totalResults: '1',
            Search: [
...
test('단일 영화의 상세 정보를 잘 가져온 경우 데이터를 확인합니다 ', async () => {
    ...
    expect(store.state.theMovies).toEqual(res.data)
})
})